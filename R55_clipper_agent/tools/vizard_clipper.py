...
Optional secrets:
    ...
    VIZARD_LANG           — Language code for subtitles (default: "de")
"""
...
def detect_vizard_video_type(url: str) -> int:
    """
    Map a video URL to Vizard's videoType code.
      2 = YouTube
      3 = Google Drive
      4 = Vimeo
      1 = direct media URL (R2, S3, MP4, etc.)
    """
    u = url.lower()
...
def submit_to_vizard(api_key: str, youtube_url: str, max_clips: int = None) -> dict:
    """
    Submit a video URL to Vizard for clipping.
    Accepts YouTube, Google Drive, Vimeo, or any direct media URL (R2, S3, MP4).
    ...
    lang = os.environ.get("VIZARD_LANG", "de")
    video_type = detect_vizard_video_type(youtube_url)
    ...
    payload = {
        ...
        "preferLength": [0, 1, 2],     # auto, 0-30s, 30-60s clips
        "subtitleSwitch": 1,
        "headlineSwitch": 1,
        "removeSilenceSwitch": 0,
    }

    # Direct media URL (R2, S3, MP4, MOV, etc.) requires the file extension.
    if video_type == 1:
...
def save_clips_to_airtable(
    ...
    records = []
    for clip in clips:
        print(f"VIZARD_CLIP_DEBUG: {json.dumps(clip, indent=2, default=str)}")
        record = {
            "fields": {
...
def post_to_blotato_platform(
    ...
    """
    Post a single clip to one Blotato platform (tiktok, instagram, or facebook).
    Blotato now accepts media URLs directly — no pre-upload required.
    """
    import requests
    ...
    body = {
        ...
        "text": caption,
        "mediaUrls": [media_url],
        "scheduledTime": scheduled_time,  # None → post immediately
    }
    ...
    # facebook: requires a Facebook Page (not personal profile) — no extra body fields
    # instagram: no extras needed
    ...
    print(f"Blotato [{platform}] response: {resp.status_code} — {json.dumps(result)[:300]}")

    if resp.status_code not in (200, 201):
        raise RuntimeError(f"Blotato {platform} failed ({resp.status_code}): {json.dumps(result)[:500]}")

    return result


def schedule_clip_all_platforms(
    api_key: str,
    accounts: dict,
    caption: str,
    media_url: str,
    ...
    """
    Fire one Blotato POST per platform (tiktok, instagram, facebook).
    `accounts` maps platform name → accountId.
    Returns dict mapping platform → result or error string.
    """
    results = {}
    for platform, account_id in accounts.items():
        if not account_id:
            results[platform] = f"skipped (no account id)"
            continue
        try:
            results[platform] = post_to_blotato_platform(
                api_key, account_id, platform, caption, media_url, scheduled_time,
            )
        except Exception as e:
            results[platform] = f"error: {str(e)[:200]}"
    return results
...
def process_video(youtube_url: str, chat_id: str, max_clips: int = None):
    ...
    try:
        ...
        send_telegram_message(
            bot_token, chat_id,
            f"🎬 <b>Vizard project created!</b>\n"
            f"📁 Project ID: <code>{project_id}</code>\n"
            f"⏱ Estimated wait: <b>5-15 minutes</b> (polling every {POLL_INTERVAL}s)"
        )
...
def process_scheduling(chat_id: str):
    ...
    platform_count = sum(1 for v in accounts.values() if v)
    ...
    try:
        ...
        send_telegram_message(
            bot_token, chat_id,
            f"📋 Found <b>{len(records)} approved clip(s)</b>. "
            f"Posting to {platform_count} platform(s) via Blotato..."
        )

        # Step 2 & 3: Post each clip to all platforms, then mark as Ready
        scheduled_clips = 0
        total_posts = 0
        errors = []
        ...
        for record in records:
            ...
            results = schedule_clip_all_platforms(
                blotato_key, accounts, caption, video_url, scheduled_time=None,
            )

            ok_platforms = [p for p, r in results.items() if isinstance(r, dict)]
            fail_platforms = [p for p, r in results.items() if not isinstance(r, dict)]
            total_posts += len(ok_platforms)
            ...
            if ok_platforms:
                ...
                try:
                    ...
                    errors.append(f"⚠️ '{title}': posted but Airtable update failed: {str(e)[:150]}")

            for p in fail_platforms:
                errors.append(f"⚠️ '{title}' [{p}]: {results[p]}")

        # Step 4: Send summary
        summary = (
            f"✅ Scheduled <b>{scheduled_clips} clips × {platform_count} platforms = "
            f"{total_posts} posts total</b>"
        )
...
def telegram_webhook(request: dict) -> dict:
    ...
    if text.strip() == "/start":
        send_telegram_message(
            ...
            "Send <code>done reviewing</code> → approved clips → Blotato → TikTok + Instagram + Facebook"
        )
        return {"ok": True}
    ...
    # --- Any URL → Vizard clipping ---
    url_match = re.search(r'(https?://[^\s]+)', text)
    if url_match:
        video_url = url_match.group(1)
        max_clips = None
        numbers = re.findall(r'\b(\d{1,3})\b', text.replace(video_url, ''))
        if numbers:
    ...
    send_telegram_message(
        ...
        "• <b>Any video URL</b> (YouTube, Drive, Vimeo, R2/S3, direct MP4) → generate clips\n"
