...
Generated assets are uploaded to fal storage to get public URLs for Airtable.
"""
...
from ..fal_upload import upload_reference
...
def _upload_base64_to_host(base64_data, filename="generated.png"):
    """
    Decode base64 image data, save to temp file, upload to fal storage.
...
def submit_image(prompt, reference_paths=None, aspect_ratio="9:16",
    ...
    Unlike Fal.ai, this returns the final GenerationResult directly
...
def poll_video(operation_name, max_wait=600, poll_interval=10, quiet=False):
    ...
    Downloads the result video and uploads to fal storage for hosting.
    ...
    while time.time() - start_time < max_wait:
        ...
        if result.get("done"):
            ...
            # Download video (requires API key auth) and upload to fal storage
...
def _download_and_host_video(video_uri):
    """Download a Veo video (requires API key) and upload to fal storage."""
