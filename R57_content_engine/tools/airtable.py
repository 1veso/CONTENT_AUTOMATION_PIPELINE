...
def _table_url():
    """Base URL for the configured table. URL-encodes the table name so
    spaces, ampersands, and other special characters in display names work."""
    table = quote(config.AIRTABLE_TABLE_NAME, safe="")
    return f"{config.AIRTABLE_API_URL}/{config.AIRTABLE_BASE_ID}/{table}"
...
def create_ugc_table():
    ...
    if response.status_code == 200:
        ...
        if "DUPLICATE_TABLE_NAME" in str(error):
            print_status(f"Table '{config.AIRTABLE_TABLE_NAME}' already exists - skipping creation", "OK")
