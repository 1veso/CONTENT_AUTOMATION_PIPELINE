...
Routes to Google AI Studio (default) or Fal.ai based on model/provider selection.

Supports:
- Nano Banana (Google: gemini-2.5-flash-image, Fal: fal-ai/nano-banana)
- Nano Banana Pro (Google: gemini-3-pro-image-preview, Fal: fal-ai/nano-banana)

Google is synchronous (no polling). Fal.ai is async (submit → poll).
"""

import time
from . import config
from .utils import print_status
from .fal_upload import upload_references
...
def generate_ugc_image(prompt, reference_paths=None, reference_urls=None,
    ...
    Args:
        ...
        reference_urls: Hosted URLs (used by Fal.ai provider)
...
def generate_for_record(record, reference_paths=None, reference_urls=None,
    ...
    Args:
        ...
        reference_urls: Pre-uploaded fal.media hosted URLs (for Fal.ai)
    ...
    if sync:
        ...
        # Fal.ai: submit all, then poll concurrently
        task_ids = []
        for var_num in var_range:
...
def generate_batch(records, reference_paths=None, model=None, provider=None,
    ...
    - ASYNC providers (Fal.ai): submit returns task_id, Phase 2 polls all concurrently.
    ...
    # Upload reference images for any async (Fal.ai) providers
    reference_urls = None
