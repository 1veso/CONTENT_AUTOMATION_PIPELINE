...
1. **Upload reference images** to fal storage (one-time, reuse URLs):
   ```python
...
- **Reference Images** attached (the fal storage hosted URL for the reference used by that specific ad)
...
# Override provider: generate_batch(records, reference_paths=[...], provider="fal")
...
- **Fal.ai** (default) — `generate_batch(pending)` (no override needed)
...
# Default (Fal.ai for Kling/Sora, Google for Veo):
...
- `provider` (optional): Override provider for Kling/Sora — `"wavespeed"` to use WaveSpeed AI instead of Fal.ai.
...
- **Image provider defaults**: Google AI Studio (can override to Fal.ai with `provider="fal"`)
- **Video provider for Kling/Sora**: Fal.ai default, WaveSpeed AI backup (`provider="wavespeed"`)
