...
# --------------------------------------------------------------- HTTP endpoints
#
# Free-tier 8-endpoint cap budget (2026-05-14):
#   vizard-clipper          → 2 (POST + GET)
#   r57-content-engine      → 2 (generate_images_http, schedule_blotato_http)
#   r61-video-pipeline      → 3 (this file, below)
#   Total                   → 7 / 8
#
# Three priority endpoints enabled — the n8n tunnel needs `hf_stitch`,
# `voiceover_gen`, and `blotato_schedule` as the user-facing entry points.
# `frame_gen_http`, `video_gen_http`, `sync_r57_to_video_http` remain offline
# (callable via `modal run` / `Function.from_name(...).remote()`) until the
# plan is upgraded.
