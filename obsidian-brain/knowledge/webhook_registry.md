...
> ⚠ **CORRECTED 2026-05-25 — the strip culprit is the n8n-mcp partial diff, NOT a verbatim REST PUT.** The `n8n_update_partial_workflow` tool re-sanitizes **ALL** nodes on every call (confirmed in its own tool docs: "auto-sanitization runs on all nodes, not just modified ones"), and that sanitizer wipes `webhook.parameters` to `{}` and clears Telegram `operation`/`resource` — the documented `n8n-mcp-sanitization-strip-bug`. A **direct REST verbatim round-trip** (GET full body → change only target fields → PUT byte-identical otherwise) does **not** strip, because every field is resent unchanged.
>
> **Always restore webhook/Telegram config via the REST verbatim PUT — never via the MCP partial diff** (the old advice here to "re-arm via `updateNode` partial-diff" was backwards and is why paths kept getting re-stripped). History on this canvas: paths wiped; "re-armed" 2026-05-15 via partial-diff (which re-stripped on later edits); fully wiped again; **restored 2026-05-25 via REST verbatim PUT** — 16 section paths + the internal `[I] Webhook` GUID + 5 Telegram `operation`/`resource`. Validation `errorCount` 37 → 15.
>
> The 2026-05-13 pre-strip backup shows the section webhooks also carried `httpMethod:"POST"` + `responseMode:"responseNode"`. The 2026-05-25 restore set **`path` only** (restoring `responseMode:"responseNode"` re-introduces a validation error unless paired with `onError:"continueRegularOutput"`). **Re-add `httpMethod:POST` + `responseMode:responseNode` + `onError:continueRegularOutput` when wiring these webhooks to live external callers.** Internal sub-webhook `[I] Webhook` (inside §I/n30 body) path = `78302243-a5e1-4e38-b0da-3bcdbc0b15b9` (auto-generated GUID, not an external entrypoint).
